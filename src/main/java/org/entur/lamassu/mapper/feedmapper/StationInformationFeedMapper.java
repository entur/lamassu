...
public class StationInformationFeedMapper implements FeedMapper<GBFSStationInformation> {
    ...
    public GBFSStationInformation map(GBFSStationInformation source, FeedProvider feedProvider) {
        ...
        mapped.setVersion(GBFSStationInformation.Version.fromValue(targetGbfsVersion));
    ...
    private GBFSStation mapStation(GBFSStation gbfsStation, String codespace) {
        ...
        mapped.setStationId(IdMappers.mapId(codespace, IdMappers.STATION_ID_TYPE, gbfsStation.getStationId()));
    ...
    private GBFSVehicleCapacity mapVehicleCapacity(GBFSVehicleCapacity vehicleCapacity, String codespace) {
        if (vehicleCapacity == null) {
            return null;
        }

        var mapped = new GBFSVehicleCapacity();
        vehicleCapacity.getAdditionalProperties().forEach((key, value) -> mapped.setAdditionalProperty(
    ...
    private GBFSVehicleTypeCapacity mapVehicleTypeCapacity(GBFSVehicleTypeCapacity vehicleTypeCapacity, String codespace) {
        if (vehicleTypeCapacity == null) {
            return null;
        }

        var mapped = new GBFSVehicleTypeCapacity();
        vehicleTypeCapacity.getAdditionalProperties().forEach((key, value) -> mapped.setAdditionalProperty(
                IdMappers.mapId(codespace, IdMappers.VEHICLE_TYPE_ID_TYPE, key),
                value
        ));
        return mapped;
