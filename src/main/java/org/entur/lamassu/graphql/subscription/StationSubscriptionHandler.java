 ...
 */
public class StationSubscriptionHandler extends EntitySubscriptionHandler<Station, StationUpdate> {
    ...
    protected boolean matchesFilter(Station station) {
        ...
        if (filterParams.getAvailableFormFactors() != null && !filterParams.getAvailableFormFactors().isEmpty()) {
            ...
            if (station.getVehicleTypesAvailable() != null) {
                for (var availability : station.getVehicleTypesAvailable()) {
                    if (filterParams.getAvailableFormFactors().contains(
                            availability.getVehicleType().getFormFactor())) {
        ...
        if (filterParams.getAvailablePropulsionTypes() != null && !filterParams.getAvailablePropulsionTypes().isEmpty()) {
            boolean hasMatchingPropulsionType = false;
            if (station.getVehicleTypesAvailable() != null) {
                for (var availability : station.getVehicleTypesAvailable()) {
                    if (filterParams.getAvailablePropulsionTypes().contains(
                            availability.getVehicleType().getPropulsionType())) {
        ...
        if (rangeParams != null) {
            ...
            return station.getLat() >= boundingBoxParams.getMinimumLatitude() &&
    ...
    @Override
    protected StationUpdate createUpdate(String id, Station station, UpdateType updateType) {
    ...
    public void initialize() {
        ...
        if (rangeParams != null) {
            initialStations = geoSearchService.getStationsWithinRange(rangeParams, filterParams);
        } else {
            initialStations = geoSearchService.getStationsInBoundingBox(boundingBoxParams, filterParams);
        }

        for (Station station : initialStations) {
            sink.tryEmitNext(new StationUpdate(station.getId(), UpdateType.CREATE, station));
        }
    }
