...
public class VehicleSubscriptionHandler
  ...
  protected boolean matchesFilter(Vehicle vehicle) {
    ...
    if (
      filterParams.getFormFactors() != null &&
      !filterParams.getFormFactors().isEmpty() &&
    ...
    if (
      filterParams.getPropulsionTypes() != null &&
      !filterParams.getPropulsionTypes().isEmpty() &&
      !filterParams
        .getPropulsionTypes()
        .contains(vehicle.getVehicleType().getPropulsionType())
    ) {
      return false;
    }
    ...
    if (rangeParams != null) {
      ...
      return (
        vehicle.getLat() >= boundingBoxParams.getMinimumLatitude() &&
  ...
  @Override
  protected List<VehicleUpdate> getInitialUpdates() {
    // Get initial data matching the subscription
    Collection<Vehicle> initialVehicles;
    if (rangeParams != null) {
      initialVehicles =
        geoSearchService.getVehiclesWithinRange(rangeParams, filterParams);
    } else {
      initialVehicles =
        geoSearchService.getVehiclesInBoundingBox(boundingBoxParams, filterParams);
    }

    System.out.println("Found " + initialVehicles.size() + " vehicles for subscription");

    // Convert to list of updates
    List<VehicleUpdate> initialUpdates = initialVehicles
      .stream()
      //.filter(this::matchesFilter)
      .map(vehicle -> new VehicleUpdate(vehicle.getId(), UpdateType.CREATE, vehicle))
      .collect(java.util.stream.Collectors.toList());

    System.out.println("Filtered to " + initialUpdates.size() + " matching vehicles");
